...
struct Token *token_create(struct Token *_token)
{
  ...
  if(lex_is_in_expression())
  {
    tmp_token.between_barackets = buffer_ptr(lex_process->parantheses_buffer);
  }
...
static bool operator_as_one(char op)
{
...
void read_op_flush_back(struct buffer *buffer)
{
...
const char *read_op()
{
  ...
  if (!single_op)
  {
    if (!op_valid(ptr))
    {
      read_op_flush_back(buffer);
      ptr[1] = 0x00;
    }
  }
  else if (!op_valid(ptr))
  {
    compiler_error(lex_process->compiler, "The operator %s is not valid", ptr);
  }
}
...
bool is_keyword(const char *str)
{
  return S_EQ(str, "unsigned") || S_EQ(str, "signed") || S_EQ(str, "char") ||
         ...
         S_EQ(str, "long") || S_EQ(str, "__ignore_typecheck") ||
         S_EQ(str, "return") || S_EQ(str, "include") || S_EQ(str, "sizeof") ||
         S_EQ(str, "if") || S_EQ(str, "else") || S_EQ(str, "while") ||
         S_EQ(str, "for") || S_EQ(str, "do") || S_EQ(str, "break") ||
         S_EQ(str, "continue") || S_EQ(str, "switch") || S_EQ(str, "case") ||
         S_EQ(str, "default") || S_EQ(str, "goto") || S_EQ(str, "typedef") ||
...
void loexer_validate_binary_string(const char *str)
{
...
struct Token *token_make_specail_number_binary()
{
  // Skip "b" ex: 0b000001
  nextc();
  unsigned long number = 0;
  const char *number_str = read_number_str();
  loexer_validate_binary_string(number_str);
...
struct Token *token_make_specail_number()
{
  ...
  {
    token = token_make_specail_number_binary();
  }
  return token;
}
